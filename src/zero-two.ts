...
const start = async () => {
		...
		if (process.env.CRON) {
			if (!cron.validate(process.env.CRON))
				return void client.log(
					`Invalid Cron String: ${chalk.bgRedBright(process.env.CRON)}`,
					true
				);
			client.log(
				`Cron Job for clearing all chats is set for ${chalk.bgGreen(
					process.env.CRON
				)}`
			);
	...
	client.on("CB:Call", async (json) => {
		...
		const number = `${
			(json[1]["from"] as string).split("@")[0]
		}@s.whatsapp.net`;
		if (!isOffer) return void null;
		client.log(
			`${chalk.blue("CALL")} From ${client.contacts[number].notify || number}`
		);
